...
public function fetch_today_deposit_monthly_comp($comp_id){
    ...
    public function fetch_today_deposit_comp($comp_id){
       ...
       public function get_today_withdrawal_monthly_comp($comp_id){
        ...
        public function get_today_withdrawal_all_comp($comp_id){
       	$today = date("Y-m-d");
       	$data = $this->db->query("SELECT SUM(loan_aprove) AS total_loanWith_all FROM tbl_loans l LEFT JOIN tbl_outstand ot ON ot.loan_id = l.loan_id WHERE l.blanch_id = '$comp_id' AND loan_stat_date = '$today'");
       	return $data->row();
       }

	   // Blanches withdrawal sessions


	   public function get_today_withdrawal_daily_blanch($blanch_id){
		$today = date("Y-m-d");
	 ...
	 public function get_today_withdrawal_weekly_blanch($blanch_id){
		$today = date("Y-m-d");
	...
	public function get_today_withdrawal_monthly_blanch($blanch_id){
		$today = date("Y-m-d");
...
public function getActiveCustomersCountByBlanchId($blanch_id) {
    // Count the number of active customers
    $this->db->select('COUNT(DISTINCT tbl_customer.customer_id) as customer_count'); // Use COUNT and DISTINCT
    $this->db->from('tbl_customer');
    $this->db->join('tbl_loans', 'tbl_customer.customer_id = tbl_loans.customer_id');
    $this->db->where('tbl_loans.loan_status', 'withdrawal');
    $this->db->where('tbl_loans.dep_status', 'open');
    $this->db->where('tbl_customer.customer_status', 'open');
    $this->db->where('tbl_customer.blanch_id', $blanch_id); // Using blanch_id to filter
    ...
    // Return the customer count, default to 0 if not found
    return isset($row->customer_count) ? $row->customer_count : 0;
}

public function getOutCustomersCountByBlanchId($blanch_id) {
    // Count the number of customers with status 'out'
    $this->db->select('COUNT(DISTINCT tbl_customer.customer_id) as customer_count'); // Count distinct customer IDs
    $this->db->from('tbl_customer');
    $this->db->join('tbl_loans', 'tbl_customer.customer_id = tbl_loans.customer_id');
    $this->db->where('tbl_customer.customer_status', 'close'); // Filter for customer status 'out'
    $this->db->where('tbl_loans.dep_status', 'out'); // Filter for loan deposit status 'out'
    $this->db->where('tbl_customer.blanch_id', $blanch_id); // Filter by blanch_id
    ...
    return isset($row->customer_count) ? $row->customer_count : 0;
	   ...
	   public function get_active_todayblanch_customer($blanch_id)
{
	$this->db->select('COUNT(customer_id) AS open_customers');
    $this->db->from('tbl_customer');
    $this->db->where('customer_status', 'open');
    $this->db->where('blanch_id', $blanch_id);
    $query = $this->db->get();

    // Fetch the result and check if there are rows
    if ($query->num_rows() > 0) {
        return (int) $query->row()->open_customers; // Ensure this is an integer
    } else {
        return 0; // No open customers
    }
}


	public function get_wadeni_sugu($blanch_id)
	{
		$date = date("Y-m-d");
		$data = $this->db->query("SELECT COUNT(d.dep_id) AS total_sugu FROM tbl_depost d LEFT JOIN tbl_customer c ON c.customer_id = d.customer_id  LEFT JOIN tbl_blanch b ON b.blanch_id = d.blanch_id WHERE d.blanch_id = '$blanch_id' AND d.depost_day = '$date' AND d.dep_status = 'out'");
		return $data->row();
	...
	public function get_pending_customer($blanch_id)
	{
		$date = date("Y-m-d");
		// Use query bindings to avoid SQL injection and add the customer_status condition
		$query = $this->db->query("SELECT COUNT(customer_id) AS total_pending FROM tbl_customer WHERE blanch_id = ? AND customer_status = ?", array($blanch_id, 'pending'));
...
public function get_total_branch_default($blanch_id)
{
	$query = $this->db->select('COUNT(customer_id) AS customer_count')
                  ->from('tbl_customer')
                  ->where('customer_status', 'out')
                  ->where('blanch_id', $blanch_id)
                  ->get();

				  $result = $query->row()->customer_count;

				  return $result;
}

public function get_customer_default_pay($blanch_id)
{
	$today = date('Y-m-d'); // Get today's date in 'YYYY-MM-DD' format

    $query = $this->db->select('COUNT(tbl_customer.customer_id) AS customer_count')
                      ->from('tbl_customer')
                      ->join('tbl_depost', 'tbl_depost.customer_id = tbl_customer.customer_id')
                      ->where('tbl_depost.dep_status', 'out')
                      ->where('tbl_depost.depost_day', $today)
                      ->where('tbl_customer.blanch_id', $blanch_id) // Filter by blanch_id
                      ->get();
...
public function get_done_customers_count($blanch_id)
{
    // Prepare the SQL query with placeholders
    $query = $this->db->query("
        SELECT COUNT(customer_id) AS total_done_customers 
        FROM tbl_customer 
        WHERE customer_status = 'close' 
        AND DATE(customer_day) = CURDATE() 
        AND blanch_id = ?", array($blanch_id));
...
public function get_total_deposits($blanch_id) {
    // Use CodeIgniter's Query Builder to avoid SQL injection and manage the query
    $this->db->select_sum('depost', 'total_depost');
    $this->db->from('tbl_depost');
    $this->db->where('DATE(depost_day)', date('Y-m-d')); // Filter by today's date
    $this->db->where('blanch_id', $blanch_id); // Filter by blanch_id
    ...
    if ($query->num_rows() > 0) {
        // Fetch the total deposit sum
        $result = $query->row();
        return $result->total_depost;
    } else {
        // Return 0 if no deposits are found
        return 0;
    }
}

public function get_total_deni($blanch_id)
{
	$date = date("Y-m-d");
		$data = $this->db->query("SELECT SUM(d.depost) AS total_sugu FROM tbl_depost d LEFT JOIN tbl_customer c ON c.customer_id = d.customer_id  LEFT JOIN tbl_blanch b ON b.blanch_id = d.blanch_id WHERE d.blanch_id = '$blanch_id' AND d.depost_day = '$date' AND d.dep_status = 'out'");
		return $data->row();
}

public function get_hai_jumla($blanch_id) {
	$this->db->select_sum('restration', 'total_restration');
	$this->db->from('tbl_loans');
	$this->db->where('loan_status', 'withdrawal');
	$this->db->where('dep_status', 'open');
	$this->db->where('blanch_id', $blanch_id); // Adding the blanch_id condition
	
	// Execute the query
	$query = $this->db->get();
	
	// Check if a result is found and return 0 for NULL
	$result = $query->row()->total_restration;
	return ($result !== NULL) ? $result : 0;
}

public function get_mauzo_out_jumla($blanch_id) {
	$this->db->select_sum('restration', 'total_restration');
	$this->db->from('tbl_loans');
	$this->db->where('loan_status', 'out');
	$this->db->where('dep_status', 'close');
	$this->db->where('blanch_id', $blanch_id); // Adding the blanch_id condition
...
public function get_total_hai($blanch_id)
{
	$date = date("Y-m-d");
		$data = $this->db->query("SELECT SUM(d.depost) AS total_hai FROM tbl_depost d LEFT JOIN tbl_customer c ON c.customer_id = d.customer_id  LEFT JOIN tbl_blanch b ON b.blanch_id = d.blanch_id WHERE d.blanch_id = '$blanch_id' AND d.depost_day = '$date' AND d.dep_status = 'withdrawal'");
		return $data->row();
}


	public function get_total_deposit_for_closed_customers($blanch_id) {
        // Prepare the SQL query with JOIN and SUM
        $this->db->select_sum('d.depost', 'total_deposit'); // Sum of deposits
        $this->db->from('tbl_depost d'); // Start from tbl_depost
        $this->db->join('tbl_customer c', 'c.customer_id = d.customer_id', 'left'); // Join on customer_id
        $this->db->where('c.customer_status', 'close'); // Filter by customer status
        $this->db->where('DATE(d.depost_day)', date('Y-m-d')); // Use deposit date for today's date
        $this->db->where('c.blanch_id', $blanch_id); // Filter by blanch_id
        
        // Execute the query
        $query = $this->db->get();
        
        // Check if the result exists and return the total deposit or 0 if NULL
        $result = $query->row();
        return ($result->total_deposit !== NULL) ? $result->total_deposit : 0;
    }

	public function get_closed_rejesho_today($blanch_id)
	{
		 // Prepare the SQL query to sum restration
		 $this->db->select_sum('l.restration', 'total_restration'); // Sum of restration
		 $this->db->from('tbl_loans l'); // Start from tbl_loans
		 
		 // Join with tbl_depost and tbl_customer
		 $this->db->join('tbl_depost d', 'l.customer_id = d.customer_id', 'left'); // Join on customer_id
		 $this->db->join('tbl_customer c', 'c.customer_id = d.customer_id', 'left'); // Join on customer_id
 
		 // Where conditions for loans
		 $this->db->where('l.loan_status', 'done'); // Filter for loan_status
		 $this->db->where('l.dep_status', 'close'); // Filter for dep_status
		 
		 // Conditions from tbl_depost and tbl_customer
		 $this->db->where('c.customer_status', 'close'); // Filter by customer status
		 $this->db->where('DATE(d.depost_day)', date('Y-m-d')); // Use deposit date for today's date
		 $this->db->where('c.blanch_id', $blanch_id); // Filter by blanch_id
		 ...
		 // Check if the result exists and return the total restration or 0 if NULL
		 $result = $query->row();
		 return ($result->total_restration !== NULL) ? $result->total_restration : 0;
	}

	public function get_open_rejesho_today($blanch_id) {
		// Prepare the SQL query to sum restration
		$this->db->select_sum('restration', 'total_restration'); // Sum of restration
		$this->db->from('tbl_loans'); // From tbl_loans
		
		// Where conditions
		$this->db->where('loan_status', 'withdrawal'); // Filter for loan_status 'withdrawal'
		$this->db->where('dep_status', 'open'); // Filter for dep_status 'open'
		$this->db->where('DATE(disburse_day)', date('Y-m-d')); // Use today's date for disburse_day
		$this->db->where('blanch_id', $blanch_id); // Filter by blanch_id
		...
		public function get_overpaid_amounts($blanch_id) {
	 // Select the sum of deposits and sum of restrictions
	 $this->db->select('SUM(d.depost) AS total_deposit, SUM(l.restration) AS total_restration');
	 $this->db->from('tbl_depost d');
	 $this->db->join('tbl_loans l', 'd.customer_id = l.customer_id', 'left');
	 $this->db->where('l.dep_status', 'open'); // Check dep_status in tbl_loans
	 $this->db->where('l.loan_status', 'withdrawal'); // Check loan_status in tbl_loans
	 $this->db->where('d.dep_status', 'withdrawal'); // Check dep_status in tbl_depost
	 $this->db->where('DATE(d.depost_day) = CURDATE()'); // Filter by today's date
	 $this->db->where('d.blanch_id', $blanch_id); // Filter by blanch_id
	 ...
	 if ($query->num_rows() > 0) {
		 $result = $query->row();
		 // Calculate the overpaid amount
		 $overpaid_amount = ($result->total_deposit - $result->total_restration);
		 return $overpaid_amount; // Return the overpaid amount
	 } else {
		 return 0; // Return 0 if no deposits are found
	 }
}

public function get_total_recevabletawi($blanch_id) {
    $date = date("Y-m-d");
    $today_data = $this->db->query("SELECT IFNULL(SUM(restration), 0) AS total_rejesho FROM tbl_loans WHERE blanch_id = '$blanch_id' AND loan_status = 'withdrawal' AND date_show = '$date' AND dep_status = 'open'");
    return $today_data->row();
}
